 ...
 */
public class ServerMessageDeserializer extends JsonDeserializer<ServerMessage<?>> {

    @Override
    public ServerMessage<?> deserialize(JsonParser jp, DeserializationContext deserializationContext) throws IOException, JacksonException {
        JsonNode node = jp.getCodec().readTree(jp);

        // 获取 type 字段并解析为 MessageType
        String typeValue = node.get("type").asText();
        ServerMessageType messageType = ServerMessageType.valueOf(typeValue.toUpperCase());
        ...
        // 创建并返回 Message 对象
        ServerMessage<BaseSocketMessage> message = new ServerMessage<>();
