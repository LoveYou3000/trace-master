...
public class TraceMasterAgent {
    ...
    public static void premain(String agentArgs, Instrumentation inst) {
        ...
        // 异步创建与 server 端的连接
        createSocketConn(agentArgs);
    }

    private static void createSocketConn(String socketServer) {
        SOCKET_EXECUTOR.execute(() -> {
            try {
                AgentSocketClient client = new AgentSocketClient(new URI("ws://" + socketServer + "/socket"));
